...
class TelitLE50Connector():
    ...
    def telit_rx_monitoring(self):
        ...
        while True:
            ...
            try:
                ...
                if (mismatch != MatchType.Handled):
                    if TELIT_ACK_OK.startswith(buf[0:len(TELIT_ACK_OK)]):
                        ...
                        if buf[0:len(TELIT_ACK_OK)] == TELIT_ACK_OK:
                            ...
                            app_log.debug("Telit LE50 (PORT: " + self.rf_connector.description + ") - Buffer Matched Telit Acknowledgement: " + repr(buf))
                        ...
                        if buf[telit_protocol_header_end_idx] != '=':
                            ...
                            if self.is_old_protocol :
                                ...
                                if len(buf) < buf_len:
                                    ...
                                    chunck_num = -1
                                    packet_id = struct.unpack('>H', buf[7: 9])[0] 
                                    rssi = struct.unpack('B', buf[12])[0]
                                    payload = buf[5: 11]

                                    self.rx_msg_subject.on_next(ChunkMonitored(unit_id, packet_id, chunck_num, payload, rssi)) 
                                    mismatch = MatchType.Handled
                                    app_log.debug("Telit LE50 (PORT: " + self.rf_connector.description + ") - Buffer Matched Pattern: unit id: " + str(unit_id) + " packet id: " + str(packet_id) + " chunk: " + str(chunck_num) + " buffer: " + to_hex(buf))
                                    buf = buf[buf_len:]
                            else:
                ...
                app_log.debug("Telit LE50 (PORT: " + self.rf_connector.description + ") before read timeout: " + str(time_out) + ", to_read: " + str(to_read))
                ch = self.rf_connector.doReceive(to_read, time_out)
                app_log.debug("Telit LE50 (PORT: " + self.rf_connector.description + ") doReceive: " + to_hex(ch))
    ...
    def SendTelitMessage(self, msg):
        ...
        return rslt
    def SendTelitMessageResponse(self, msg):

        self.telit_ack_event.clear()

        self.rf_connector.flushAll()
        self.rf_connector.doSend(msg)
        rslt = self.rf_connector.doReceiveCR(1)
        app_log.info("Telit LE50 (PORT: " + self.rf_connector.description + ") - Send Message: " + str(msg) + "Response:" + rslt)
        return rslt
